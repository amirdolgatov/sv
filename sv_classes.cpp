...
void Container:: add(Attribute& element) {
    if(element.length <= 128) {                                  ///добавление метки не требуется
        ...
        std::cout << " case element.length < 128" << std::endl;
    }
    else if (element.length > 128 && element.length < 256) {     ///требуется добавление метки 0x81
        this->length += (element.length + element.tagSize + element.lengthSize);
        this->tag[1] = 0x81;
        this->tagSize = 2;
        std::cout << " case element.length >= 128 && element.length < 256" << std::endl;
    }
        ...
        std::cout << " case element.length > 256" << std::endl;
    }
...
SV_attributes SV_generator:: initialize_ASDU(SV_attribute_values& source){
    this->asdu_attr.svID = String_attribute(source.svID, 0x80);
    this->asdu_attr.datSet = String_attribute(source.datSet, 0x81);
    this->asdu_attr.smpCnt = Type_attribute<uint16_t>(source.smpCnt, 0x82);
    this->asdu_attr.confrev = Type_attribute<uint32_t>(source.confrev, 0x83);
    this->asdu_attr.refrTm = Type_attribute<uint64_t>(source.refrTm, 0x84);
    this->asdu_attr.smpSynch = Type_attribute<uint8_t>(source.smpSynch, 0x85);
    this->asdu_attr.smpRate = Type_attribute<uint16_t>(source.smpRate, 0x86);
    this->asdu_attr.seq_of_Data = Seq_of_Data(0x87);
